 ...
 * @Version: 0.1.0
 * @Author: AiDongYang
 * @Date: 2021-01-28 15:36:59
 * @LastEditors: AiDongYang
 * @LastEditTime: 2021-04-29 16:09:20
-->
<template>
  <div>
    <SearchFormBox>
      <ElForm ref="searchForm" :model="form" :rules="rules">
        ...
        <ElFormItem label="型号" prop="modelCode" class="required-item">
          <ModelSelect ref="model" v-model="form.modelCode" :type="MODEL_NAME.CHARGE_BOARD" @getList="getModelList" />
        </ElFormItem>

        <ElFormItem label="硬件版本" prop="version" class="required-item">
          <HardwareVersion v-model="form.version" :type="MODEL_NAME.CHARGE_BOARD" />
        </ElFormItem>
...
import { getChargeBoardQueryList, chargeBoardQueryDownload } from 'src/api/deviceTest'
  ...
  methods: {
    ...
    getList() {
      ...
      this.$refs.searchForm.validate(async valid => {
        if (valid) {
          const { pageInfo, testItem } = await getChargeBoardQueryList(this.handleParams())
    ...
    download() {
      ...
      this.$refs.searchForm.validate(async valid => {
        if (valid) {
          const data = await chargeBoardQueryDownload(this.handleParams())
