 ...
 * @Version: 0.1.0
 * @Author: AiDongYang
 * @Date: 2021-01-28 15:13:30
 * @LastEditors: AiDongYang
 * @LastEditTime: 2021-04-22 14:57:38
-->
    ...
    <SearchFormBox>
      <ElForm ref="searchForm" :model="form">
        ...
        <ElFormItem label="型号" prop="modelCode" class="required-item">
          <ModelSelect ref="model" v-model="form.modelCode" :type="MODEL_NAME.CHARGE_BOARD" multiple />
        </ElFormItem>
...
import { getChargeBoardStatisticsList, chargeBoardStatisticsDownload, chargeBoardStatisticsDataContrast } from 'src/api/deviceTest'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      // 获取列表
      const { data = [], dateList = [] } = await getChargeBoardStatisticsList(this.handleParams())
    ...
    async download() {
      // 下载数据
      const data = await chargeBoardStatisticsDownload(this.handleParams())
    ...
    async compare({ checkedModels, checkedFields }) {
      ...
      // if (flag) {
      //   const res = await chargeBoardStatisticsDataContrast({
