 ...
 * @Version: 0.1.0
 * @Author: AiDongYang
 * @Date: 2021-01-28 15:08:33
 * @LastEditors: AiDongYang
 * @LastEditTime: 2021-04-22 14:57:45
-->
<template>
  <div>
    <SearchFormBox>
      <ElForm ref="searchForm" :model="form">
        ...
        <ElFormItem label="型号" prop="modelCode" class="required-item">
          <ModelSelect ref="model" v-model="form.modelCode" :type="MODEL_NAME.COMMUNICATION_BOARD" multiple />
        </ElFormItem>
    ...
    <TableListBox>
      <template>
        <div slot="right">
          ...
          <FieldFilter
            type-name="硬件版本"
            :data="hardwareVersionListFilter"
...
import { MODEL_NAME, DATA_FIELDS } from 'src/common/constants'
import { getCommunicationBoardStatisticsList, communicationBoardStatisticsDownload } from 'src/api/deviceTest'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      // 获取列表
      const { data = [], dateList = [] } = await getCommunicationBoardStatisticsList(this.handleParams())
    ...
    async download() {
      // 下载数据
      const data = await communicationBoardStatisticsDownload(this.handleParams())
    ...
    async compare({ checkedModels, checkedFields }) {
      // console.log(checkedModels, checkedFields)
      // 数据对比
      // if (flag) {
