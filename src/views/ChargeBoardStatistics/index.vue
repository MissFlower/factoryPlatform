 ...
 * @Version: 0.1.0
 * @Author: AiDongYang
 * @Date: 2020-11-30 17:26:17
 * @LastEditors: AiDongYang
 * @LastEditTime: 2021-03-08 16:56:41
-->
<template>
  <div>
    <SearchFormBox>
      <ElForm ref="searchForm" :model="form">
        ...
        <ElFormItem label="硬件版本" prop="hardwareVersion">
          <HardwareVersion ref="hardware" v-model="form.hardwareVersion" :type="MODEL_NAME.CHARGE_BOARD" multiple />
        </ElFormItem>
...
import { chargeBoard, chargeBoardDownload, chargeBoardContrast } from '@/api/chargeBoard'
...
export default {
  ...
  methods: {
    ...
    async getList() {
      // 获取列表
      const data = await chargeBoard(this.returnParams())
    ...
    async download() {
      // 下载数据
      const data = await chargeBoardDownload(this.returnParams())
      downLoadExcel(data)
    },
    dealData() {
      let contactDot = 0
    ...
    async compare(data, flag) {
      ...
      if (flag) {
        const res = await chargeBoardContrast({
          startDate: form.date[0],
          endDate: form.date[1],
